...
export class SchemaManager {
    ...
    /**
     * Generate CREATE TABLE SQL for a table (utility function)
     * @param tableName Name of the table
     * @param dialect SQL dialect ('postgres' | 'mysql' | 'sqlite')
     * @returns CREATE TABLE SQL statement
     */
    public generateCreateTableSQL(tableName: string, dialect: 'postgres' | 'mysql' | 'sqlite' = 'postgres'): string {
        const table = this.schemas[tableName];
        if (!table) {
        ...
        Object.entries(table.columns).forEach(([columnName, column]) => {
            ...
            switch (column.type) {
                case 'string':
                    const maxLength = column.constraints?.maxLength || 255;
                    line += dialect === 'postgres' ? ` VARCHAR(${maxLength})` : ` VARCHAR(${maxLength})`;
                    break;
                case 'number':
                    line += dialect === 'postgres' ? ' INTEGER' : ' INT';
                    break;
                case 'boolean':
                    line += dialect === 'postgres' ? ' BOOLEAN' : ' BOOLEAN';
                    break;
                case 'date':
                    line += dialect === 'postgres' ? ' TIMESTAMP' : ' DATETIME';
                    break;
                case 'json':
                    line += dialect === 'postgres' ? ' JSONB' : ' JSON';
                    break;
                case 'uuid':
                    line += dialect === 'postgres' ? ' UUID' : ' VARCHAR(36)';
                    break;
            }
            ...
            if (column.defaultValue !== undefined) {
                line += ` DEFAULT ${typeof column.defaultValue === 'string' ? `'${column.defaultValue}'` : column.defaultValue}`;
            }
        ...
        lines.push(...columnLines.map((line, index) =>
            line + (index < columnLines.length - 1 ? ',' : '')
        ));
        lines.push(');');
