...
- Do not hand-edit `tests/ztd-layout.generated.ts`; regenerate it with `pnpm playground:gen-config` (or `pnpm --filter ztd-playground exec ztd ztd-config`) so the CLI and tests stay aligned.
- `tests/ztd-row-map.generated.ts` (TestRowMap + table schemas) is generated by `pnpm playground:gen-config` and is the canonical source for typed fixtures.
