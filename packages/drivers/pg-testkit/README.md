...
Housing schema files under a dedicated folder (for example, `sql/ddl/schemas/users.sql`, `sql/ddl/schemas/orders.sql`) makes it obvious which tables exist and where seed data originates. Each `.sql` file can contain `CREATE TABLE` statements plus optional `INSERT` statements, and the loader turns those inserts into fixture rows so your tests reuse the seeds declared next to the schema.
