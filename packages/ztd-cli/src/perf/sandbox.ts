...
export function resolvePerfExternalDatabaseUrl(env: NodeJS.ProcessEnv = process.env): string | null {
  const explicitUrl = (env.ZTD_DB_URL ?? '').trim();
...
export async function ensurePerfConnection(rootDir: string, config: PerfSandboxConfig): Promise<{ connectionUrl: string; usedDocker: boolean }> {
  ...
  if (!existsSync(composeFile)) {
    if (ignoredDefaultDatabaseUrl) {
      throw new Error('Perf sandbox ignores DATABASE_URL for ZTD-owned workflows. Set ZTD_DB_URL explicitly or run `ztd perf init` first.');
    }
