...
async function runLintCommand(pattern: string): Promise<void> {
  ...
  const databaseUrl = process.env.ZTD_DB_URL?.trim();
  ...
  try {
    ...
    if (!resolvedConnectionUrl) {
      const containerModule = await ensurePostgresContainerModule().catch((error) => {
        ...
        throw new Error(`${baseMessage} Or set ZTD_DB_URL to reuse an existing Postgres connection.`);
      });
...
function assertDockerReadyForLint(): void {
  ...
  if (probe.error || probe.status !== 0) {
    ...
    throw new Error(
      `Docker is not reachable. Start Docker Desktop/service before running ztd lint without ZTD_DB_URL.${detail}`
    );
  }
}
...
export function buildLintContainerStartError(error: unknown): Error {
  ...
  if (message.toLowerCase().includes('container runtime strategy') || message.toLowerCase().includes('docker')) {
    return new Error(
      `${message} Start Docker Desktop/service, or set ZTD_DB_URL to use an existing Postgres.`
    );
  }
...
export function buildLintConnectionError(error: unknown, usingExternalConnection: boolean): Error {
  ...
  const guidance = usingExternalConnection
    ? 'Check ZTD_DB_URL and verify the ZTD-owned test database is reachable.'
    : 'Check Docker Desktop/service and retry, or set ZTD_DB_URL to skip container startup.';
