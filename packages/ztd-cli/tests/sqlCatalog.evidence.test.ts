...
describe('sql catalog evidence', () => {
  it('is deterministic with stable sorting and case-level params', () => {
    ...
    expect(exported1.catalogs[0]?.cases).toEqual([
      {
        ...
        params: { active: 1, limit: 2, minTotal: 20 },
        expected: [
      ...
      },
      {
        ...
        params: { active: 0, limit: 2, minTotal: 20 },
        expected: [{ orderId: 12, userEmail: 'bob@example.com', orderTotal: 40 }],
      },
    ]);
    expect(exported1.catalogs[0]?.sql).toBe(activeOrdersSqlCases.catalog.sql);
