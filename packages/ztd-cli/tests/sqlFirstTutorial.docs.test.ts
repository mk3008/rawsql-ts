...
test('the tutorial preserves the shortest DDL to first test path', () => {
  ...
  expectInOrder(tutorial, [
    ...
    'SQL repair | `npx ztd model-gen --probe-mode ztd src/features/users-insert/queries/insert-users/insert-users.sql --out src/features/users-insert/queries/insert-users/spec.ts`',
  ...
  expect(tutorial).toContain('queries/insert-users/tests/insert-users.queryspec.ztd.test.ts');
  ...
  expect(tutorial).toContain('Keep `src/features/users-insert/spec.ts`, the query-local `src/features/users-insert/queries/insert-users/spec.ts`, and the query-local SQL resource inside `src/features/users-insert/queries/insert-users`.');
  ...
  expect(tutorial).toContain('Before writing the success-path assertion, inspect `insert-users.sql` and `spec.ts`. If the scaffold does not actually return a non-null id, report that mismatch instead of inventing fixture data or schema overrides.');
  expect(tutorial).toContain('When the cases are ready, run `npx vitest run src/features/users-insert/queries/insert-users/tests/insert-users.queryspec.ztd.test.ts` to execute the ZTD query test.');
