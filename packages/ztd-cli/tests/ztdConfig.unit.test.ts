...
test('preserves identity column defaults through the runtime fixture manifest', () => {
  const sources: SqlSource[] = [
    {
      path: 'DDL/users.sql',
      sql: `
  ...
  expect(tables[0]?.columns).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        name: 'user_id',
        defaultValue: 'row_number() over ()'
      })
    ])
  );

  const manifestOutput = normalizeLineEndings(renderZtdFixtureManifestFile(tables));
  expect(manifestOutput).toContain('name: "user_id"');
  expect(manifestOutput).toContain('defaultValue: "row_number() over ()"');
});
