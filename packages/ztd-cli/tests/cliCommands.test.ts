...
test('query slice emits the minimal dependency chain for a target CTE', () => {
  const workspace = createSqlWorkspace('query-slice-cte', path.join('src', 'sql', 'reports', 'purchase_summary.sql'));
  writeFileSync(
    workspace.sqlFile,
    `
      with users_base as (
  ...
  const result = runCli(['query', 'slice', workspace.sqlFile, '--cte', 'purchase_summary'], {}, workspace.rootDir);
  assertCliSuccess(result, 'query slice cte');
  expect(result.stdout).toContain('"users_base" as');
  expect(result.stdout).toContain('"filtered_users" as');
  expect(result.stdout).toContain('"purchase_summary" as');
  expect(result.stdout).toContain('from "purchase_summary"');
  expect(result.stdout).not.toContain('final_projection as');
  expect(result.stdout).not.toContain('unused_cte as');
});

test('query slice can preserve the minimized final query', () => {
  const workspace = createSqlWorkspace('query-slice-final', path.join('src', 'sql', 'reports', 'final_query.sql'));
  writeFileSync(
    workspace.sqlFile,
    `
      with base_data as (
  ...
  const result = runCli(['query', 'slice', workspace.sqlFile, '--final'], {}, workspace.rootDir);
  assertCliSuccess(result, 'query slice final');
  expect(result.stdout).toContain('"base_data" as');
  expect(result.stdout).toContain('"filtered_data" as');
  expect(result.stdout).toContain('from "filtered_data"');
  expect(result.stdout).not.toContain('unused_data as');
});

test('query slice adds LIMIT to the emitted debug query', () => {
  const workspace = createSqlWorkspace('query-slice-limit', path.join('src', 'sql', 'reports', 'limited_slice.sql'));
  writeFileSync(
    workspace.sqlFile,
    `
      with base_data as (
  ...
  const result = runCli(['query', 'slice', workspace.sqlFile, '--cte', 'target_data', '--limit', '25'], {}, workspace.rootDir);
  assertCliSuccess(result, 'query slice limit');
  expect(result.stdout).toContain('from "target_data"');
  expect(result.stdout).toContain('limit 25');
});

test('query slice supports DML final slices while removing unused CTEs', () => {
  const workspace = createSqlWorkspace('query-slice-insert-final', path.join('src', 'sql', 'reports', 'insert_report.sql'));
  writeFileSync(
    workspace.sqlFile,
    `
      with source_rows as (
  ...
  const result = runCli(['query', 'slice', workspace.sqlFile, '--final'], {}, workspace.rootDir);
  assertCliSuccess(result, 'query slice insert final');
  expect(result.stdout).toContain('"source_rows" as');
  expect(result.stdout).toContain('insert into');
  expect(result.stdout).toContain('"user_report"');
  expect(result.stdout).not.toContain('audit_rows as');
});
