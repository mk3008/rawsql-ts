...
- a DB-backed smoke test that uses `createStarterPostgresTestkitClient` from `.ztd/support/postgres-testkit.ts` on top of `@rawsql-ts/testkit-postgres` and checks `ZTD_TEST_DATABASE_URL` connectivity
