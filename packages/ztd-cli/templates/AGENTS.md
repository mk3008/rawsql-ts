...
- When a request says `ZTD-format tests`, treat that as query-boundary-local SQL verification through the project's fixed app-level ZTD harness against the real database engine, not a mocked executor.
- Feature-boundary tests are mock-based and live in `src/features/<feature>/tests/<feature>.boundary.test.ts`.
- Query-boundary tests own the ZTD lane, colocated under each query directory with `tests/<query>.boundary.ztd.test.ts`, `tests/generated/`, and `tests/cases/`.
