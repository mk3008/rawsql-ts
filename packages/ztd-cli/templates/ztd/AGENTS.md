...
- The `public.user_account` and `public.user_profile` tables exist to support the SQL-first samples; any modification to those tables is a maintenance obligation that requires concurrent updates to `src/sql/user_account/`, `src/repositories/views/user-profiles.ts`, `src/repositories/tables/user-accounts.ts`, and the repository tests so the workflow keeps functioning.
