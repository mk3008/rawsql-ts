...
The starter keeps `ztdRootDir`, `ddlDir`, and the `ddl.defaultSchema` / `ddl.searchPath` defaults in `ztd.config.json`, so the helper reads the project defaults from one place instead of repeating them in every DB-backed test.
