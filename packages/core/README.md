...
- **Configurable Export**: Choose `exportComment` = "full", "none", "header-only" (only header annotations), or "leaf-header-only" (outermost header only) to control comment emission
...
- **Performance Optimized**: Applies conditions in the most efficient order (filter ↁEsort ↁEpaginate ↁEserialize)
...
- About 3 Ex faster than `node-sql-parser`.
- About 4 Ex faster than `sql-parser-cst`.
- About 7 Ex faster than `sql-formatter`.
