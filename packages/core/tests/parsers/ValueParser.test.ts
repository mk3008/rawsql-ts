...
describe('ValueParser', () => {
    ...
    test.each([
        ...
        ["EXTRACT - Extract month from timestamp", "EXTRACT(MONTH FROM '2025-03-21 12:34:56'::timestamp)", "extract(month from cast('2025-03-21 12:34:56' as timestamp))"],
        ...
        ["Postgres CAST with AS syntax", "id::INTEGER", "cast(\"id\" as INTEGER)"],
        ["Postgres CAST with precision", "price::NUMERIC(10,2)", "cast(\"price\" as NUMERIC(10, 2))"],
        ["Postgres CAST with length", "name::VARCHAR(50)", "cast(\"name\" as VARCHAR(50))"],
        ["CAST with DOUBLE PRECISION", "value::DOUBLE PRECISION", "cast(\"value\" as DOUBLE PRECISION)"],
        ["CAST with AS syntax and namespaced type", "CAST(id AS myschema.INTEGER)", "cast(\"id\" as \"myschema\".INTEGER)"],
        ["Postgres CAST with :: and namespaced type", "id::myschema.INTEGER", "cast(\"id\" as \"myschema\".INTEGER)"],
        ["CAST with CHARACTER VARYING", "text::CHARACTER VARYING(100)", "cast(\"text\" as CHARACTER VARYING(100))"],
        ["CAST with TIME WITH TIME ZONE", "ts::TIME WITH TIME ZONE", "cast(\"ts\" as TIME WITH TIME ZONE)"],
        ["CAST with TIMESTAMP WITHOUT TIME ZONE", "date::TIMESTAMP WITHOUT TIME ZONE", "cast(\"date\" as TIMESTAMP WITHOUT TIME ZONE)"],
        ["Postgres CAST with INTERVAL", "'1 month'::interval", "cast('1 month' as interval)"],
        ...
        ["AT TIME ZONE - timestamp literal", "'2025-03-28 15:30:00'::timestamp AT TIME ZONE 'America/New_York'", "cast('2025-03-28 15:30:00' as timestamp) at time zone 'America/New_York'"],
        ["AT TIME ZONE - nested", "('2025-03-28 15:30:00'::timestamp AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Tokyo'", "(cast('2025-03-28 15:30:00' as timestamp) at time zone 'UTC') at time zone 'Asia/Tokyo'"],
        ...
        ["ANY subquery comparison", "status = ANY (SELECT status FROM allowed_order_statuses)", "\"status\" = any(select \"status\" from \"allowed_order_statuses\")"],
        ["SOME subquery comparison", "status = SOME (SELECT status FROM allowed_order_statuses)", "\"status\" = some(select \"status\" from \"allowed_order_statuses\")"],
        ["ALL subquery comparison", "status <> ALL (SELECT status FROM blocked_order_statuses)", "\"status\" <> all(select \"status\" from \"blocked_order_statuses\")"],
        ["Row constructor ANY subquery comparison", "(customer_id, status) = ANY (SELECT customer_id, status FROM allowed_order_statuses)", "(\"customer_id\", \"status\") = any(select \"customer_id\", \"status\" from \"allowed_order_statuses\")"],
        ["ANY array comparison", "status = ANY (:statuses)", "\"status\" = any(:statuses)"],
        ["SOME array comparison", "status = SOME (ARRAY['paid', 'shipped'])", "\"status\" = some(array['paid', 'shipped'])"],
        ["ALL array comparison", "status <> ALL (:blocked_statuses)", "\"status\" <> all(:blocked_statuses)"],
        ["FunctionCall with arithmetic operation", "count(*) + 1", "count(*) + 1"],
        ["Postgres Array Type", ":array::int[]", "cast(:array as int[])"],
        ["Postgres Array Type - Single Item", ":item::int", "cast(:item as int)"],
        ["Postgres Array Type - Multi-dimensional", ":array::int[][]", "cast(:array as int[][])"],
        ...
        ["Type cast - parameterized NUMERIC", '"value"::NUMERIC(10,2)', 'cast("value" as NUMERIC(10, 2))'],
        ["Type cast - parameterized VARCHAR", '"name"::VARCHAR(50)', 'cast("name" as VARCHAR(50))'],
        ["Type cast - parameterized DECIMAL", '"data"::DECIMAL(8,4)', 'cast("data" as DECIMAL(8, 4))'],
        ["Type cast - parameterized TIMESTAMP", '"timestamp"::TIMESTAMP(6)', 'cast("timestamp" as TIMESTAMP(6))'],
        ["Type cast - namespaced type", '"id"::pg_catalog.int4', 'cast("id" as "pg_catalog".int4)'],
        ["Type cast - public schema type", '"value"::public.custom_type', 'cast("value" as "public".custom_type)'],
        ["Type cast - complex expression", '(x + y)::INTEGER', 'cast(("x" + "y") as INTEGER)'],
        ["Type cast - CASE expression", 'CASE WHEN a > b THEN a ELSE b END::BIGINT', 'cast(case when "a" > "b" then "a" else "b" end as BIGINT)'],
        ["Type cast - multiple casts", '"a"::INTEGER::TEXT', 'cast(cast("a" as INTEGER) as TEXT)'],
        ["Type cast - with parentheses", '("col1" || "col2")::VARCHAR(100)', 'cast(("col1" || "col2") as VARCHAR(100))'],
        ["Type cast - array type", '"id"::int[]', 'cast("id" as int[])'],
        ["Type cast - multi-dimensional array", '"data"::int[][]', 'cast("data" as int[][])'],
        ["Type cast - type without parameters", '"value"::NUMERIC', 'cast("value" as NUMERIC)'],
        ["Type cast - literal cast", '123::TEXT', 'cast(123 as TEXT)'],
        ["Type cast - function cast", 'NOW()::DATE', 'cast(now() as DATE)'],
