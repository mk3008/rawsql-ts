...
describe('SqlPaginationInjector', () => {
    test('should inject pagination for first page', () => {
        // Arrange
        ...
        const expectedSql = 'select "id", "name" from "users" limit 20';
    ...
    test('should inject pagination for second page', () => {
        // Arrange
        const injector = new SqlPaginationInjector();
        const baseQuery = 'SELECT id, name FROM users WHERE active = true';
        const pagination: PaginationOptions = { page: 2, pageSize: 10 };
        const expectedSql = 'select "id", "name" from "users" where "active" = true limit 10 offset 10';
    ...
    test('should calculate correct offset for higher page numbers', () => {
        // Arrange
        const injector = new SqlPaginationInjector();
        const baseQuery = 'SELECT * FROM products';
        const pagination: PaginationOptions = { page: 5, pageSize: 25 };
        const expectedSql = 'select * from "products" limit 25 offset 100';
    ...
    test('should preserve existing ORDER BY clause', () => {
        // Arrange
        const injector = new SqlPaginationInjector();
        const baseQuery = 'SELECT id, name FROM users ORDER BY created_at DESC';
        const pagination: PaginationOptions = { page: 3, pageSize: 15 };
        const expectedSql = 'select "id", "name" from "users" order by "created_at" desc limit 15 offset 30';
    ...
    test('should handle complex queries with all clauses', () => {
        // Arrange
        const injector = new SqlPaginationInjector();
        const baseQuery = `
        ...
        const expectedSql = 'select "u"."id", "u"."name", "p"."title" from "users" as "u" join "posts" as "p" on "u"."id" = "p"."user_id" where "u"."active" = true and "p"."published" = true group by "u"."id", "u"."name", "p"."title" having count(*) > 1 order by "u"."name", "p"."title" desc limit 50 offset 50';
    ...
    test('should handle large page numbers correctly', () => {
        // Arrange
        const injector = new SqlPaginationInjector();
        const baseQuery = 'SELECT id FROM users';
        const pagination: PaginationOptions = { page: 100, pageSize: 50 };
        const expectedSql = 'select "id" from "users" limit 50 offset 4950';
    ...
    test('should handle single item per page', () => {
        ...
        const expectedSql = 'select "id" from "users" limit 1 offset 4';
    ...
    test('should accept maximum allowed page size', () => {
        // Arrange
        const injector = new SqlPaginationInjector();
        const baseQuery = 'SELECT id FROM users';
        const pagination: PaginationOptions = { page: 1, pageSize: 1000 };
        const expectedSql = 'select "id" from "users" limit 1000';
