    ...
    describe('Complex Mixed Relationships', () => {
        it('should handle complex mixed relationships: Order > OrderLines[] > Product and Order > Customer', () => {
            ...
            const expectedSql = [
                ...
                `            *`,
                `            , case`,
                `                when "product_id" is null and "product_name" is null then`,
                `                    null`,
                `                else`,
                ...
                `            , "customer_name"`,
                `            , "customer_email"`,
                `            , jsonb_agg(jsonb_build_object('id', "line_id", 'quantity', "line_quantity", 'price', "line_price", 'product', "product_json_1")) as "lines"`,
                `        from`,
                `            "cte_object_depth_1"`,
                `        group by`,
                `            "customer_json_2"`,
                ...
                `    , "cte_root_orders" as (`,
                `        select`,
                `            jsonb_build_object('id', "order_id", 'date', "order_date", 'status', "order_status", 'lines', "lines", 'customer', "customer_json_2") as "Orders"`,
                `        from`,
                `            "cte_array_depth_1"`,
                `    )`,
                `select`,
