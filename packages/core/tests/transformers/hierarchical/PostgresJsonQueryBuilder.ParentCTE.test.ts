...
describe("PostgresJsonQueryBuilder - Parent Entity CTE Generation", () => {
    it("should generate parent CTE for customer entity", () => {
        ...
        const expectedSql = [
            ...
            `            *`,
            `            , case`,
            `                when "customer_id" is null and "customer_name" is null and "customer_email" is null then`,
            `                    null`,
            `                else`,
    ...
    it("should generate parent CTEs for complex hierarchy with multiple branches", () => {
        ...
        // 1. origin_query
        // 2. cte_object_depth_2 (Address and Carrier)
        // 3. cte_object_depth_1 (Customer and ShippingInfo)
        ...
        const expectedSql = [
            ...
            `            "order_details_view"`,
            `    )`,
            `    , "cte_object_depth_2" as (`,
            `        select`,
            `            *`,
            `            , case`,
            `                when "address_id" is null and "address_street" is null and "address_city" is null and "address_zip" is null then`,
            `                    null`,
            `                else`,
            `                    jsonb_build_object('id', "address_id", 'street', "address_street", 'city', "address_city", 'zip', "address_zip")`,
            `            end as "address_json_1"`,
            `            , case`,
            `                when "carrier_id" is null and "carrier_name" is null and "carrier_phone" is null then`,
            `                    null`,
            `                else`,
            ...
            `            , case`,
            `                when "shipping_id" is null and "shipping_method" is null and "shipping_fee" is null then`,
            `                    null`,
            `                else`,
