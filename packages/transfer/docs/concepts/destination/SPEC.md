...
Destination は、赤伝生成に必要な転送先側の符号反転列情報を持ってよい。

たとえば、赤伝生成時に符号反転する数量や金額などの列を示してよい。

`Red Transfer` を成立させるには、数量や金額など、少なくとも1つの符号反転列が必要である。

赤伝は既に転送されている元黒から作るため、符号反転列は転送先側の黒伝をどう反転するかを示す情報である。

赤伝は元黒行を反転する転送であり、赤伝へ引き継ぐ列を個別に定義する概念ではない。

これは赤伝を生成するための転送先側の仕様であり、具体的な SQL 生成手順や実行手順ではない。

### Generated Red Transfer SQL Belongs to Destination

`Red Transfer` は、既に転送されている元黒行を転送先側で反転して生成する。

そのため、AOT 生成された `Red Transfer` SQL を保持する場合、その所属先は `Destination Link` ではなく Destination である。

`Destination Link` は source-to-destination mapping を扱うが、赤伝は転送元SQLの結果列を送る処理ではない。
赤伝SQLは、Destination の転送先テーブル、キー列、transfer model、符号反転列、必要な日付下限制御など、転送先側の仕様に基づく。

これは実行時のSQLを事前にレビュー可能な形で保持するための所属境界であり、具体的なSQL生成手順ではない。
...
採番式は転送先仕様であり、Destination 側で管理する。

`Transfer Setting` の基礎 SQL は、Destination が示す採番式を使って `destination row key` を投影してよい。

transfer engine は、データソースクエリを転送するだけであり、基礎 SQL に採番式や `destination row key` を暗黙に追加しない。
...
- Destination の transfer model、赤伝列情報、生成済み赤伝SQLは、後続の SQL 生成や転送実行の前提として扱う。
- 日付下限制御の詳細は `Date Lower Bound Control` で扱う。Destination は補正関数名と転送先行の引数列を参照するが、締め管理テーブルや締め判定ロジックそのものは所有しない。
