...
- Work Item は、transfer model による route 選択と、その route 内でどの転送操作が副作用を持つかを判断できる情報を持つ。
...
- Work Item は、`Transfer Execution` が immutable route または mutable route を選び、route 内の `Black Insert Transfer`、`Black Update Transfer`、`Red Transfer`、`Physical Delete Transfer` が副作用を持つかを判断できるようにする。
...
- Work Item は、同じ `Transfer Setting`、`Destination Link`、`source key` の文脈で既に同じ転送判断として扱われた重複 `Dirty Key` を無視できる。
- Work Item の処理済み記録は、`Dirty Key Processing` に残す。
...
- Work Item は、転送 route 選択と route 内の副作用有無に必要な判断情報を持つ。これがない場合、`Transfer Execution` はどの route でどの転送操作を成立させるべきかを判断できない。
- `Transfer Execution` は、Work Item に整理された情報をもとに immutable route または mutable route を選び、route 内で必要な転送操作だけを成立させる。
...
この転送判断は、transfer model による route 選択と、その route 内で転送操作が副作用を持つかを判断できる情報である。

この文書が固定するのは具体的な判定式ではなく、判断に必要な概念参照が Work Item に揃うことである。
...
Work Item は、この分類名を重複した enum として持つ必要はない。transfer model、転送元の現在値の有無、`Active Black` の有無、重複判定などの判断材料から、route と副作用有無を判断できればよい。
...
`Dirty Key Processing` は、`Dirty Key` ID、`Destination Link` ID、処理結果、`Transfer Run` を記録し、`Transfer Setting` と `Destination Link` の文脈で処理済みを追跡できる必要がある。
...
- Work Item は、転送判断を持つ。
- Work Item の転送判断は、transfer model による route 選択と route 内の副作用有無を決める判断情報として読む。
- Work Item の判断は、`Dirty Key`、転送元の現在値、`Active Black`、transfer model に依存する。
- Work Item は、`Dirty Key` 自体に処理済み状態を書き込まない。
- Work Item は、処理済み記録の正本を `Dirty Key Processing` に置く。
- `Transfer Execution` は、Work Item に整理された情報をもとに immutable route または mutable route を選び、必要な転送操作だけを成立させる。
