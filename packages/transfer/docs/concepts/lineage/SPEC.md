...
- Lineage は、`destination table` の転送先行から、その行を生成または記録した転送元情報をたどれるようにする。
...
- Lineage は、どの `Destination Link` の文脈で destination row が生成されたかを追跡できる。
...
- Lineage は、どの `Destination Link` の文脈で生成された destination row かを参照できる。
- Lineage は、`Destination Link` を通じてどの `Destination` への転送かを参照できる。
...
- Lineage が保証するのは `source key` と destination row の対応関係であり、転送元の値スナップショットそのものではない。
...
`Black Transfer` では、`source key` は転送時に参照された転送元データソースの論理行または再評価単位を識別する。
物理テーブルの1行とは限らない。
...
1つの `source key` から複数の destination row が生成されることはある。
たとえば、1つの仕訳 source row から、借方の科目残高 row と貸方の科目残高 row が生成される場合がある。

この場合でも、借方の destination row から見た `source key` は1つであり、貸方の destination row から見た `source key` も1つである。

そのため、転送先行から転送元への Lineage は `0..1` として扱う。

同じ `Destination` に複数の destination row を生成する場合でも、Lineage は `Destination Link` の文脈で destination row を区別する。
...
- Lineage は、`destination table` の行から、その転送元と実行文脈をたどるために使う。
...
- Lineage は、`Destination Link` を参照することで、同じ `Destination` に対する複数の宛先別 row role を区別する。
