...
`Destination Link` ID がないと、ある `Dirty Key` がどの宛先別設定で処理済みなのかを区別できず、二重転送防止の粒度が粗くなる。
...
- `Work Item` の処理が終わったら、Dirty Key Processing に `Dirty Key` ID、`Destination Link` ID、処理結果、`Transfer Run` を記録する。
