...
## Hardcoded Parameter Detection Limitations

`DynamicQueryBuilder` relies on `ParameterDetector` to separate hardcoded placeholders from runtime filters. That detector only recognizes placeholders when they appear in simple binary comparisons such as `table.column = :param`. If your SQL wraps a column reference inside arithmetic, functions, or any other expression, the AST no longer links the column directly to the placeholder, so no column-to-parameter mapping is produced. Filters whose SQL counterparts use those expressions therefore stay in the dynamic category and must be kept as runtime inputs (or rewritten back into simple comparisons). Refer to `packages/core/tests/utils/ParameterDetector.test.ts` for the regression test that locks in this limitation.

When you have a choice, the named placeholder style is easier to reason about because the filter payload keys match the columns you are targeting. DynamicQueryBuilder keeps treating indexed placeholders as if they were keyed by the assignment column, but the AST can only recover those column names when the comparison stays simple. Arithmetic, casts, or other expressions hide the target column and lead to unresolved placeholders, so the index style becomes fragile except on very plain queries. Drivers such as `pg` that expose only positional parameters therefore interact poorly with hardcoded placeholders; you still have to write the simple comparisons, but you should expect tighter restrictions on when DynamicQueryBuilder can detect the mapping.

The builder now fails fast when any remaining placeholder lacks a value. `ParameterDetector.separateFilters` throws an error along the lines of `Missing values for hardcoded placeholders (:target)` so that `SqlParameterBinder` never sees `null` or `undefined` values. Make sure every placeholder in your base SQL is backed by a filter entry (matching the placeholder name) or remove the placeholder before passing the SQL to `DynamicQueryBuilder`; otherwise the operation raises a clear error. Because the reranking process reassigns numbers to indexed placeholders and still relies on column names for the exposed filters, you cannot satisfy the hardwired parameters by writing `filter: { '$1': 10 }`; instead always name the column you are filtering (`filter: { id: 10 }`) so that the builder can align the dynamic additions with the rewritten SQL.
