...
| Result row mapping | Generated code | AOT row mapper generated by `ztd-cli` |
| Query SQL file loading | Generated code | Generated boundary-local resource loader |
...
The generated `FeatureQueryExecutor` type has a minimal contract:
...
try {
  ...
  const executor = async (sql: string, params: Record<string, unknown>) => {
  ...
  await executeRenameUserQuerySpec(executor, { userId, newName });
